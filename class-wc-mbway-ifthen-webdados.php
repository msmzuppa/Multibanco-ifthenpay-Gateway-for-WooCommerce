...
if ( ! class_exists( 'WC_MBWAY_IfThen_Webdados' ) ) {
	...
	class WC_MBWAY_IfThen_Webdados extends WC_Payment_Gateway {
		...
		public function __construct() {
			...
			$this->icon = ( $this->get_option( 'small_icon' ) == 'yes' ? plugins_url( 'images/icon_mbway_48_small.png', __FILE__ ) : plugins_url( 'images/icon_mbway_48.png', __FILE__ ) );
...
Email enviado automaticamente do plugin WordPress “Multibanco, MBWAY and Payshop (IfthenPay) for WooCommerce” para '.$to.' com CC para '.$cc;
		...
		public function get_icon() {
			...
			$icon_html = ( $this->get_option( 'small_icon' ) == 'yes' ? '<img src="'.esc_attr( $this->icon ).'" alt="'.esc_attr( $alt ).'" width="26" height="24"/>' : '<img src="'.esc_attr( $this->icon ).'" alt="'.esc_attr( $alt ).'" width="50" height="24"/>' );
					...
					if ( $callback_email_sent == 'no' ) {
						if ( !isset( $_GET['callback_warning'] ) ) {
							if ( apply_filters( 'mbway_ifthen_show_callback_notice', true ) ) {
								?>
								<div id="mbway_ifthen_callback_notice" class="error notice" style="padding-right: 38px; position: relative;">
									<p>
